...
class CalculateRestOfTimeoff extends Controller
{
    ...
    public function execute(Request $request = null)
    {
        ...
        $dailyUsed = Timeoffs::where('user', $userId)->where('type', 'روزانه')->where('approved', 1)->sum('duration');
        $hourlyUsed = Timeoffs::where('user', $userId)->where('type', 'ساعتی')->where('approved', 1)->sum('duration');
