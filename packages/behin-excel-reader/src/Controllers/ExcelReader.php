    ...
    */
    ...
    public function unzip($filename, $is_data = false)
    {
        ...
        if ($is_data) {
            ...
            $this->package['size'] = ExcelReader::strlen($filename);
...
//          $bE = false;
            ...
            if ($aP['GPF'] & 0x0008) {
                $aP1 = unpack('V1CRC/V1CS/V1UCS', ExcelReader::substr($vZ, -12));

                $aP['CRC'] = $aP1['CRC'];
                $aP['CS'] = $aP1['CS'];
                $aP['UCS'] = $aP1['UCS'];
                // 2013-08-10
                $vZ = ExcelReader::substr($vZ, 0, -12);
                if (ExcelReader::substr($vZ, -4) === "\x50\x4b\x07\x08") {
                    $vZ = ExcelReader::substr($vZ, 0, -4);
                }
            }

            // Getting stored filename
            $aI['N'] = ExcelReader::substr($vZ, 26, $nF);
            $aI['N'] = str_replace('\\', '/', $aI['N']);

            if (ExcelReader::substr($aI['N'], -1) === '/') {
            ...
            $vZ = ExcelReader::substr($vZ, 26 + $nF + $mF);

            if ($aP['CS'] > 0 && (ExcelReader::strlen($vZ) !== (int)$aP['CS'])) { // check only if availabled
                $aI['E'] = 1;
...
//        $m1 = 0; // memory_get_peak_usage( true );
        ...
        if ($relations = $this->getEntryXML('_rels/.rels')) {
            foreach ($relations->Relationship as $rel) {
                ...
                $rel_target = ExcelReader::getTarget('', (string)$rel['Target']); // /xl/workbook.xml or xl/workbook.xml
                ...
                if ($rel_type === 'officeDocument'
                    ...
                    if ($workbookRelations = $this->getEntryXML(dirname($rel_target) . '/_rels/workbook.xml.rels')) {
                        ...
                        foreach ($workbookRelations->Relationship as $workbookRelation) {
                            ...
                            $wrel_target = ExcelReader::getTarget(dirname($rel_target), (string)$workbookRelation['Target']);
                                ...
                                if ($sharedStrings = $this->getEntryXML($wrel_target)) {
                                    foreach ($sharedStrings->si as $val) {
                                        if (isset($val->t)) {
                                            ...
                                            $this->sharedstrings[] = ExcelReader::parseRichText($val);
                                        }
                                    }
                                }
...
//            echo round( ($m2-$m1) / (1024 * 1024), 2).' MB'.PHP_EOL;
    ...
    public function getEntryData($name)
    {
        ...
        $dir = ExcelReader::strtoupper(dirname($name));
        $name = ExcelReader::strtoupper(basename($name));
        foreach ($this->package['entries'] as &$entry) {
            if (ExcelReader::strtoupper($entry['path']) === $dir && ExcelReader::strtoupper($entry['name']) === $name) {
                if ($entry['error']) {
                    return false;
                }
                switch ($entry['cm']) {
                ...
                if (!$entry['error'] && $entry['cm'] > -1) {
                    ...
                    } elseif ($entry['ucs'] > 0 && (ExcelReader::strlen($entry['data']) !== (int)$entry['ucs'])) {
                        $entry['error'] = 3;
 ...
 // 0.6.6
    ...
    public function dimension($worksheetIndex = 0)
    {
        ...
        if (ExcelReader::strpos($ref, ':') !== false) {
            $d = explode(':', $ref);
...
//      $this->error( 'Invalid cell index ' . $cell );
    ...
    public function value($cell)
    {
        ...
        switch ($dataType) {
            ...
            case 'inlineStr':
                ...
                $value = ExcelReader::parseRichText($cell->is);
    ...
    public function readRowsEx($worksheetIndex = 0, $limit = 0)
    {
        if (!$this->rowsExReader) {
            ...
            $this->rowsExReader = new ExcelReader($this);
        }
    ...
    public function getCell($worksheetIndex = 0, $cell = 'A1')
    {
        ...
        if (is_array($cell)) {
            $cell = ExcelReader::num2name($cell[0]) . $cell[1];// [3,21] -> D21
        }
        if (is_string($cell)) {
    ...
    public static function getTarget($base, $target)
    {
        $target = trim($target);
        if (strpos($target, '/') === 0) {
            return ExcelReader::substr($target, 1);
        }
    ...
    public static function num2name($num)
    {
        ...
        $num2 = (int)(($num - 1) / 26);
        if ($num2 > 0) {
            return ExcelReader::num2name($num2) . $letter;
        }
        return $letter;
    }
